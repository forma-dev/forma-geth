...
interface IStrings {
  ...
  function replace(string memory _str, string memory _old, string memory _new, uint16 _n) external pure returns (string memory);

  /// @dev Replaces all occurrences of a substring in a string
  function replaceAll(string memory _str, string memory _old, string memory _new) external pure returns (string memory);
