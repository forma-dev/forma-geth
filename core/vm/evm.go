...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile {
		ret, gas, err = evm.precompileManager.Run(addr, input, caller.Address(), value, gas, evm.Config.Tracer, false)
	} else {
...
func (evm *EVM) DelegateCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	// It is allowed to call precompiles, even via delegatecall
	if isPrecompile := evm.precompileManager.IsPrecompile(addr); isPrecompile {
		ret, gas, err = evm.precompileManager.Run(addr, input, caller.Address(), nil, gas, evm.Config.Tracer, false)
	} else {
		addrCopy := addr
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile := evm.precompileManager.IsPrecompile(addr); isPrecompile {
		ret, gas, err = evm.precompileManager.Run(addr, input, caller.Address(), new(uint256.Int), gas, evm.Config.Tracer, true)
	} else {
